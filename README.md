...
1. Убедитесь, что в директориях SRC/BJRD и/или SRC/XBJRD находятся текстовые файлы словаря и другие исходные файлы:
    * **SRC/BJRD/bjrd.txt** и/или **SRC/BJRD/xbjrd.txt** - текстовый файл словаря публичной (без ※-комментариев) или редакторской (с ※-комментариями) версии соответственно, кодировка UTF-16LE. Обратите внимание, что файл должен называться соответственно корпусу: bjrd или xbjrd. Названия остальных файлов ниже не зависят от корпуса и называются для всех корпусов одинаково.
...
3. Для генерации публичной версии BJRD запустите скрипт **bjrd.bat**. После окончания его работы в папке HTML/BJRD окажутся все файлы, необходимые для генерации словаря с помощью EBStudio. Файл BJRD.map окажется в папке EPWING/BJRD. В файле BJRD_encoding_report.html будет записан отчет о конвертации гайдзи.

4. Запустите скрипт **EBStudio.bat**. Этот скрипт производит корректный запуск EBStudio\EBStudio.exe через AppLoc.exe.

5. В открывшемся окне EBStudio выберите пункт меню ファイル（F） -> 開く(O). Выберите файл bjrd.ebs. Все поля EBStudio автоматически заполнятся нужными значениями.

6. В меню EBStudio выберите пункт ファイル（F） -> 実行(G) или нажмите кнопку с красным восклицательным знаком. После окончания работы процедуры конвертации в папке EPWING/BJRD окажется полностью скомпилированный словарь BJRD и файл BJRD.map.

> Для редакторской версии словаря процедура генерации аналогична. На шаге **3** следует запускать **xbjrd.bat**, файлы для генерации с помощью EBStudio поступят в папку HTML/XBJRD. На шаге **5** следует выбирать файл **xbjrd.ebs**, результат поступит в папку EPWING/XBJRD.
...
1. Производится сборка всех файлов HTML для EBStudio (dict.html и copyright.html) в кодировке UTF-16LE.
...
11. Производится генерация 3 файлов, описывающих гайдзи:
    ...
    * **BJRD.map** - содержит соответствие ebcode-кодов *реальным* юникод-кодам. В ebcode-коде префиксом h/z указана ширина символа.
12. По окончании генерируется файл отчета обработки гайдзи **BJRD_encoding_report.html**, который содержит перечень всех гайдзи в виде таблицы. В таблице указаны:
    * Порядковый номер гайдзи
    * Ebcode-код, присвоенный гайдзи, включая префикс h/z, обозначающий ширину
    * Реальный юникод-код символа (для символов из субприватной зоны словаря здесь будет указан код \&#xE000-\&#xE020)
    * Юникод-код символа так, как он встречается в файлах dict.html и copyright.html (то есть для пятизначных кодов здесь будет код из PUA U+E021–U+F8FF).
    ...
    * Сам символ. Для символов из субприватной зоны словаря (\&#xE000-\&#xE020) символ будет выведен в браузере. В некоторых браузерах такой несуществующий символ будет отображаться как квадратик, а в некоторых - как ромбик или стрелочках (это изображения этих символов из наиболее распространенных PUA).
